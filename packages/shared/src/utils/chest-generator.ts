...
} from "../constants/chest_const";
import { ITEM_POOLS } from "../constants/items/item_pool";

export type ChestGeneratorOptions = {
  rng?: () => number;
};
...
export class ChestGenerator {
   ...
   */
  static generateRandomChest(options: ChestGeneratorOptions = {}): ChestInfo {
    const rng = options.rng ?? defaultRng;
    const chestType = this.weightedRandom(CHEST_RARITY_WEIGHTS, rng);
  ...
  static openChest(
    ...
    if (eggChance && rng() * 100 < eggChance) {
      eggDropped = true;
    }
  ...
  private static generateGuaranteedItems(
    ...
    for (let i = 0; i < config.guaranteed_items; i++) {
      ...
      const itemType =
        availableTypes[Math.floor(rng() * availableTypes.length)];
      usedTypes.push(itemType!);

      // Генерувати item
      const item = this.generateItem(itemType!, config, rng);
      items.push(item);
    }
  ...
  private static generateBoostItem(
    ...
    return this.generateItem("boost", config, rng);
  }
  ...
  private static generateItem(
    ...
    const itemRarity = this.weightedRandom(config.item_rarity_distribution, rng);
    ...
    if (!pool || pool.length === 0) {
      ...
      const itemId =
        fallbackPool[Math.floor(rng() * fallbackPool.length)]!;
      const quantity = this.determineQuantity(fallbackRarity, rng);
    ...
    const quantity = this.determineQuantity(itemRarity, rng);
   ...
   */
  private static determineQuantity(
    rarity: RarityType,
    rng: () => number,
  ): number {
