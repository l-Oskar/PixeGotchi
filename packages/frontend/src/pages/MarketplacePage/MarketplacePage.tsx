...
const MarketplacePage: React.FC<MarketplacePageProps> = () => {
  ...
  const sellableCosmetics =
    roomCosmeticsOwnership.data?.cosmetics.filter(
      ({ quantity, asset }) =>
        quantity > 0 && asset?.isTradable === true && asset.isDefault === false,
    ) ?? [];
  ...
  const handleCreateEgg = () => {
    createEgg.mutate(undefined, {
      onSuccess: () => {
        showSuccess({
          message: "You bought an egg!",
        });
      },
      onError: (error) => {
        showApiError(error, { title: "Egg fail" });
      },
    });
  };
  ...
  const handleAddItem = (itemId: string, quantity?: number) => {
    addItem.mutate(
      { itemId, quantity },
      {
        onSuccess: () => {
          showSuccess({
            message: `Item ${itemId} purchased!`,
          });
        },
        onError: (error) => {
          showApiError(error, { title: "Item fail" });
        },
      },
    );
  };
            ...
            <div className="pixel-panel-soft mt-2 grid gap-2 p-2">
              ...
              <div className="grid grid-cols-[minmax(0,1fr)_5.5rem] gap-2">
                <select
                  ...
                  onChange={(event) =>
                    setSellCosmeticAssetId(event.target.value)
                  }
