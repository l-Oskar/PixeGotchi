...
function isScheduleDue(schedule, hour) {
  const startHour = Number(schedule.startHour ?? schedule.hour ?? 0);
  const repeatEveryHours = Number(schedule.repeatEveryHours ?? 0);
  const untilHour = Number(schedule.untilHour ?? Infinity);
...
function getUsageCount(usage, itemId, now) {
  return usage.dailyCounts.get(`${itemId}:${dayKey(now)}`) ?? 0;
}

function incrementUsageCount(usage, itemId, now) {
  const key = `${itemId}:${dayKey(now)}`;
  usage.dailyCounts.set(key, (usage.dailyCounts.get(key) ?? 0) + 1);
}

function getItemEffect(item, stat) {
  return Number(item.effects?.[stat] ?? 0);
}

function getBoostAllStats(item) {
  return (item.effects?.buffs ?? []).reduce(
    (sum, buff) => sum + Number(buff.boost_all_stats ?? 0),
    0,
  );
}

function buildEffects(item) {
  const boostAllStats = getBoostAllStats(item);

  return Object.fromEntries(
    STAT_KEYS.map((key) => [
      key,
      getItemEffect(item, key) + (boostAllStats > 0 ? boostAllStats : 0),
    ]),
  );
}

function canUseItem(item, usage, now) {
  const lastUsedAt = usage.lastUsedAt.get(item.itemId);
  const cooldownMinutes = Number(item.cooldownMinutes ?? 0);

  if (lastUsedAt && cooldownMinutes > 0) {
    const nextAllowedAt = lastUsedAt + cooldownMinutes * 60_000;
    if (now.getTime() < nextAllowedAt) {
      return {
        allowed: false,
        reason: `cooldown until ${new Date(nextAllowedAt).toISOString()}`,
      };
    }
  }

  if (item.maxPerDay !== null && item.maxPerDay !== undefined) {
    const usedToday = getUsageCount(usage, item.itemId, now);
    if (usedToday >= Number(item.maxPerDay)) {
      return {
        allowed: false,
        reason: `daily limit ${item.maxPerDay}`,
      };
    }
  }
...
function applyScheduledItems(state, snapshot, scenario, itemMap, usage, hour, now, shared) {
  const dueSchedules = (scenario.itemSchedule ?? []).filter((schedule) =>
    isScheduleDue(schedule, hour),
  );
  ...
  for (const schedule of dueSchedules) {
    ...
    if (!item) {
      events.push({
        ...
        label: schedule.label ?? schedule.itemId,
        blocked: true,
        reason: "item not found",
        effects: {},
        statsAfter: Object.fromEntries(STAT_KEYS.map((key) => [key, round(Number(next[key]))])),
        statusAfter: next.status,
      });
      continue;
    }

    const usageCheck = canUseItem(item, usage, now);
    if (!usageCheck.allowed) {
      events.push({
        hour,
        type: item.itemType,
        itemId: item.itemId,
        label: schedule.label ?? item.name,
        blocked: true,
        reason: usageCheck.reason,
        effects: buildEffects(item),
        statsAfter: Object.fromEntries(STAT_KEYS.map((key) => [key, round(Number(next[key]))])),
        statusAfter: next.status,
      });
      continue;
    ...
    const applied = applyItemToSnapshot(next, item, now, shared);
    next = applied.snapshot;
    usage.lastUsedAt.set(item.itemId, now.getTime());
    incrementUsageCount(usage, item.itemId, now);
...
function summarizeSeries(series, startMs) {
  ...
  for (const point of series) {
    ...
    for (const action of point.actions ?? []) {
      const countTarget = action.blocked ? blockedCounts : actionCounts;
      countTarget[action.type] = (countTarget[action.type] ?? 0) + 1;
      itemCounts[action.itemId] = (itemCounts[action.itemId] ?? 0) + (action.blocked ? 0 : 1);
      actionEvents.push({
        hour: point.hour,
  ...
  if (blockedTotal > 0) {
    verdictReasons.push(`${blockedTotal} scheduled item uses were blocked by cooldown or daily limits`);
  }
  if (Object.keys(actionCounts).length > 0 && outcome !== "survived") {
    verdictReasons.push("configured item care pattern was insufficient");
  }
  if (verdictReasons.length === 0) {
    verdictReasons.push("item care pattern survived the configured window");
  }
...
function simulateScenario(scenario, config, context, constantOverrides, itemMap) {
  ...
  for (let elapsedMs = 0; elapsedMs <= totalMs; elapsedMs += stepMs) {
    ...
    const { snapshot: finalSnapshot, events } = applyScheduledItems(
      state,
      snapshotWithTimers,
      scenario,
      itemMap,
  ...
  return {
    ...
    careActions: scenario.itemSchedule ?? [],
    itemSchedule: scenario.itemSchedule ?? [],
...
export const simulationModule = {
  id: "care-balance",
  title: "Care Balance Simulation",
  run(config, context) {
    const constantOverrides = buildConstantOverrides(config);
    const itemMap = new Map(context.shared.ALL_ITEMS.map((item) => [item.itemId, item]));
    ...
    return {
      ...
      scenarios: (config.scenarios ?? []).map((scenario) =>
        simulateScenario(scenario, config, context, constantOverrides, itemMap),
      ),
    };
  },
  toJson(result) {
